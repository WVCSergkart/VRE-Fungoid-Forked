...
namespace VanillaRacesExpandedFungoid
{
    ...
    public static class VanillaRacesExpandedFungoid_MentalStateHandler_TryStartMentalState_Patch
    {
...

        {
            ...
            if(__result)
            {
                if(___pawn.genes?.HasActiveGene(InternalDefOf.VRE_MindCoalescence)==true && stateDef != MentalStateDefOf.SocialFighting && stateDef != InternalDefOf.Crying && stateDef != InternalDefOf.Giggling)
                {
